...
public class formKhuyenMai extends javax.swing.JPanel {
    ...
    public void loadTableKhuyenMai(List<KhuyenMai> khuyenMais) {
    ...
    private void checkEditingStatus(JTable table) {
     ...
     private boolean isValidIdKhuyenMai(String idKMStr) {

        if (idKMStr == null) {
            return false;
        }
        if (idKMStr.trim().isEmpty()) {
            return false;
        }
        try {
            int idKM = Integer.parseInt(idKMStr.trim());
            return idKM > 0;
        } catch (NumberFormatException e) {
            return false;
        }
    }
     public void loadAllKhuyenMai() {
        List<KhuyenMai> lstKM = khuyenMaiService.getAll();
        model.setRowCount(0);
    ...
    private void initComponents() {
        ...
        jLabel10.setPreferredSize(new java.awt.Dimension(164, 20));
    ...
    private void btnTimKiemSanPhamActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTimKiemSanPhamActionPerformed
        String idKMStr = txtTimKiem.getText();
        if (idKMStr == null) {
            JOptionPane.showMessageDialog(this, "ID khuyến mãi không được null");
            return;
        }
        if (idKMStr.trim().isEmpty()) {
        ...
        if (!isValidIdKhuyenMai(idKMStr)) {
            JOptionPane.showMessageDialog(this, "ID khuyến mãi phải là một số nguyên dương hợp lệ");
            return;
        }
        int idKM = Integer.parseInt(idKMStr.trim());
        List<KhuyenMai> khuyenMais = khuyenMaiService.timKiemKhuyenMai(idKM);

        if (khuyenMais.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Không tìm thấy khuyến mãi với ID: " + idKM);
            loadAllKhuyenMai();
        } else {
            loadTableKhuyenMai(khuyenMais);
        }
